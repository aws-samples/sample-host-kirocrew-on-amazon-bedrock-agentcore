...
- **绑定令牌** 授权每次调用，有效期 30 分钟；沙箱租约存活期间会透明续签，长时间打开的
  页面不会因此失效，也不会轮换会话。容器赢得沙箱初始化后，Broker 会为它签发一枚绑定同一
  沙箱与会话的运行时会话令牌（上限为平台会话生命周期），因此浏览器离开后心跳与后台检查点
  仍可继续。
- **microVM 的执行角色不持有任何数据访问权限。** 沙箱内的一切——包括用户自己的终端——都
